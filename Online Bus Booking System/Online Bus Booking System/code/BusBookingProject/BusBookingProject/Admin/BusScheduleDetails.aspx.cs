...
namespace BusBookingProject.Admin
{
    public partial class BusScheduleDetails : System.Web.UI.Page
    {
        ...
        private int addBusScheduleData()
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["RouteID"]) && int.TryParse(Request.QueryString["RouteID"], out int routeID))
            {
                sqlCmd.Parameters.AddWithValue("@RouteID", routeID); // Add @RouteID parameter
            }
            else
            {
                // Handle the case where RouteID is not provided or is not a valid integer
                // For example:
                Response.Write("RouteID not provided or is invalid.");
