        ...
        private int getBook()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < seatArray.Length; i++)
                {
                    ...
                    int boardingID;
                    if (int.TryParse(Request.QueryString["BoardingID"], out boardingID))
                    {
                        sqlCmd.Parameters[11].Value = boardingID;
                    }
                    else
                    {
                        // Handle the case where the BoardingID is not a valid integer
                        // You could set a default value or log the issue for debugging
                        // For example:
                        sqlCmd.Parameters[11].Value = 5;/* Set a default boarding ID or handle the error */;
                        // Log or display a message about the invalid boarding ID
                    }
